...
def test_model_training_and_serialization():
    ...
    assert metrics.mape_test < 8.0  # stochastic ML tolerance — MAPE can fluctuate
...
def test_model_api_endpoints(client, auth_header):
    """Verify REST API model training, status, and prediction endpoints."""
    # 1. Train endpoint
    r_train = client.post("/api/v1/model/train", headers=auth_header)
    ...
    # 2. Status endpoint
    r_status = client.get("/api/v1/model/status", headers=auth_header)
    ...
    # 3. Predict endpoint
    r_pred = client.get("/api/v1/model/predict?horizon_days=14", headers=auth_header)
