...
def test_esankhyiki_api_endpoints(client, auth_header):
    """Verify all eSankhyiki REST API endpoints return HTTP 200 and expected schemas."""
    # 1. Metadata
    r_meta = client.get("/api/v1/esankhyiki/metadata", headers=auth_header)
    assert r_meta.status_code == 200
    assert r_meta.json()["monitored_group"]["group_code"] == "6.1.03"

    # 2. Baseline
    r_base = client.get("/api/v1/esankhyiki/cpi-baseline", headers=auth_header)
    assert r_base.status_code == 200
    assert r_base.json()["group_code"] == "6.1.03"

    # 3. Augmented CPI
    r_aug = client.get("/api/v1/esankhyiki/augmented-cpi", headers=auth_header)
    assert r_aug.status_code == 200
    assert "augmented_nowcast_cpi" in r_aug.json()

    # 4. Sync
    r_sync = client.post("/api/v1/esankhyiki/sync", headers=auth_header)
