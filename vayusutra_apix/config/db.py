...
from .routes import DGCA_TOP_20_ROUTES
...
class DatabaseManager:
    ...
    def init_db(self) -> None:
        ...
        with self._lock:
            ...
            # Seed normalized routes registry from the statutory DGCA Top-20 basket
            cursor.execute("SELECT COUNT(*) as cnt FROM routes_registry")
            if cursor.fetchone()[0] == 0:
                now_iso = datetime.datetime.now(datetime.timezone.utc).isoformat()
                default_routes = [
                    (
                        r.route_code, r.origin, r.destination, r.origin_city,
                        r.destination_city, r.weight, r.distance_km,
                        1 if r.is_metro_metro else 0, r.base_fare_benchmark, 5, now_iso
                    )
                    for r in DGCA_TOP_20_ROUTES
                ]
                cursor.executemany("""
                    INSERT INTO routes_registry (
                        route_code, origin, destination, origin_city, destination_city,
                        dgca_weight, distance_km, is_metro_metro, base_benchmark_fare,
                        active_carriers_count, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, default_routes)
