...
class RouteConsensusRecord:
    ...
    data_tag: str = "REAL_COMPUTED"   # REAL_COMPUTED or MODELLED (synthesized when no observations exist)
...
class SourceConsensusReport:
    ...
    generated_at: str
    real_corridors_analyzed: int = 0      # Corridors scored from actual observations only
    modelled_corridors_analyzed: int = 0  # Corridors synthesised from base benchmarks (no observations)
...
class SourceConsensusEngine:
    ...
    def analyze_consensus(self, target_date: Optional[str] = None) -> SourceConsensusReport:
        ...
        for r in DGCA_TOP_20_ROUTES:
            ...
            record_tag = "REAL_COMPUTED"
            real_count += 1

            if not q_list:
                # Explicitly MODELLED substitute when a corridor has no observations
                # for the requested date. Never passed off as real observed data.
                record_tag = "MODELLED"
                real_count -= 1
                modelled_count += 1
        ...
        real_scores = [c.consensus_score for c in corridor_records if c.data_tag == "REAL_COMPUTED"]
        avg_market_consensus = round(float(np.mean(real_scores)), 1) if real_scores else 0.0
        ...
        return SourceConsensusReport(
            ...
            real_corridors_analyzed=real_count,
            modelled_corridors_analyzed=modelled_count
        )
